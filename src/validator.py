...
class ComputeValidator(Module):
    ...
    def validate_workloads(self,
        ...
        # def is_invalid_result(result_dict):
        #     global required_fields
        #     return any([field not in result_dict for field in required_fields])
        ...
        for worker_info in list(validated_workloads.keys()):
            ...
            if any([field not in result for field in required_fields]):
                ...
                if "failure_reason" not in worker_info_to_result_dict[worker_info]:
                    worker_info_to_result_dict[worker_info]["failure_reason"] = "Missing required fields"
        ...
        def filter_result_len_capacity(result_dict, capacity):
            ...
            return equal_length and match_capacity

        for worker_info in list(validated_workloads.keys()):
            result = validated_workloads[worker_info]
            if not filter_result_len_capacity(result, worker_capacity_dict[worker_info]):
                validated_workloads.pop(worker_info)
                if "failure_reason" not in worker_info_to_result_dict[worker_info]:

                    worker_info_to_result_dict[worker_info]["failure_reason"] = "Invalid result length"
    ...
    def validation_loop(self) -> None:
        while True:
            ...
            logger.info(f"Validation loop took {elapsed_time:.2f} seconds, sleeping for {self.interval} seconds.")
            if elapsed_time < 30:
...
def main():
    ...
    parser.add_argument('--subnet_uid', type=int, default=8, help='Subnet UID, default is 26 for testnet')
